# -*- coding: utf-8 -*-
"""
Created on Thu Sep  5 15:27:43 2019
...
#######TELL US WHICH CELLS WERE JUST PULSED. THESE CELLS ARE NEWLY EXCITED AND NOT EXCITABLE############
    ...
    def propagate(self, t, T):
        ...
        self.prev_time_of_exc[np.nonzero(new_excited_cells)] = self.new_time_of_exc[np.nonzero(new_excited_cells)]
        self.new_time_of_exc[np.nonzero(new_excited_cells)] = t
        
        time_diff=(self.new_time_of_exc-self.prev_time_of_exc)*(self.no_of_times_cell_excited > 1)*new_excited_cells
...
#        test.send_wave(5000,T)
    test=Lattice(L=L, tau=tau, nux=nux, nuy=nuy, delta=delta, epsilon=epsilon, draw=animate) 
    test.send_wave(10000, T)
    plt.plot(test.t, test.avg_list, 'g')
    plt.title(f'nu_x = {nux}, nu_y = {nuy}')
    #plt.ylim(105,115)
...
if animate:
    ...
    ani=animation.FuncAnimation(fig, animate, frames=np.arange(0,10000,1), init_func=init, interval=0, repeat=False, blit=0)
